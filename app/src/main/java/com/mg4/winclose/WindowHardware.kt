...
object WindowHardware {
    ...
    // Katman4 SWI69 — CarAdapterClient → CarVehicleSettingClient (vitres)
    ...
    // Katman5 SWI69 — CarAdapterClient → CarGeneralClient (ignition + vitesse)
    ...
    // CarStateClient (BIND_CODE_CAR_STATE = 11) — gear + parking brake en temps réel
    ...
    private const val CAR_STATE_CALLBACK_INTERFACE  = "com.saicmotor.carapi.carstate.ICarStateCallback"
    private const val TX_GEAR_CHANGE         = 3
    private const val TX_PARKING_BRAKE       = 7
    private const val TX_SERVICE_READY       = 1
    // Sur ce MG4 : gear=1 quand levier en P, doorSensor=0 quand porte ouverte
    private const val GEAR_PARK              = 1
    private const val TX_DOOR_SENSOR         = 6   // onSensorEptrdyStateChanged
    private const val DOOR_OPEN              = 0   // v=0 = porte ouverte (observé)

    // Les seuils sont désormais configurables via Settings.*
    // Cooldown anti-spam entre 2 triggers
    ...
    object CarIgnitionItem {
        ...
        const val RUN       = 0x2
        const val CRANK     = 0x3  // Sur MG4 : porte conducteur ouverte / parking forcé
    }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Speed monitor — tracking de la vitesse pour valider sHasBeenDriving
    // ─────────────────────────────────────────────────────────────────────────
        ...
        Thread {
            ...
            while (speedMonitorActive) {
                try {
                    ...
                    sLastSpeed = speed
                    val threshold = speedThresholdKmh()
                    if (!sHasBeenDriving && speed >= threshold) {
                        sHasBeenDriving = true
                        log("hasBeenDriving=true (vitesse ${"%.1f".format(speed)} ≥ $threshold km/h)")
                    }
                } catch (_: Exception) {}
                Thread.sleep(500)
            }
    ...
    private fun initKatman4(context: Context) {
        ...
        val adapter = try {
            ...
            log("Katman4: getInstance err: ${e.message}")
            return
        } ?: run {
            Handler(Looper.getMainLooper()).postDelayed({ initKatman4(context) }, 10_000)
            return
        }
    ...
    private fun tryInitVsm(adapter: Any, adapterClass: Class<*>, clientClass: Class<*>) {
        ...
        try {
            ...
            val client = clientClass.getConstructor(android.os.IBinder::class.java).newInstance(ibinder)
            sVsm = client
            log("VSM prêt ✓")
        } catch (e: Exception) {
            Log.d(TAG, "tryInitVsm err: ${e.message}")
        }
    }

    // ─────────────────────────────────────────────────────────────────────────
    // Katman5 — CarGeneralClient (ignition + state 2↔3)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    private fun initKatman5(context: Context) {
        ...
        val adapter = try {
            ...
            log("Katman5: getInstance err: ${e.message}")
            return
        } ?: return
    ...
    private fun tryRegisterIgnitionListener(
        adapter: Any, adapterClass: Class<*>, generalClass: Class<*>
    ) {
        ...
        try {
            ...
            val callbackBinder = object : Binder() {
                override fun onTransact(code: Int, data: Parcel, reply: Parcel?, flags: Int): Boolean {
                    if (code == TX_IGNITION_CHANGE) {
                        try {
                            data.enforceInterface(IGNITION_CALLBACK_DESCRIPTOR)
                            dispatchIgnition(data.readInt())
                        } catch (_: Exception) {}
                    }
            ...
            Log.d(TAG, "tryRegisterIgnitionListener err: ${e.message}")
        }
    }
    ...
    private fun dispatchIgnition(state: Int) {
        ...
        when (state) {
            ...
            CarIgnitionItem.RUN -> {
                // Armer le timer 5 min (la vitesse > 20 km/h reste l'autre condition)
                if (!sHasBeenDriving) armDrivingTimer()
            }
        }

        val cbs = ignitionCallbacks.toList()
        Handler(Looper.getMainLooper()).post { cbs.forEach { it(state) } }
    }

    // ─────────────────────────────────────────────────────────────────────────
    // CarStateClient — gear + parking brake en temps réel (BIND_CODE = 11)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    private fun initCarState(context: Context) {
        ...
        val adapter = try {
            ...
            log("CarState: getInstance err: ${e.message}")
            return
        } ?: return
    ...
    private fun tryRegisterCarStateListener(
        adapter: Any, adapterClass: Class<*>, stateClass: Class<*>
    ) {
        ...
        try {
                    ...
                    try {
                        when (code) {
                            TX_SERVICE_READY -> {
                                ...
                                log("CarState: onServiceReady")
                            }
                            TX_GEAR_CHANGE -> {
                                data.enforceInterface(CAR_STATE_CALLBACK_DESCRIPTOR)
                                val gear = data.readInt()
                                log("CarState: gear → $gear${if (gear == GEAR_PARK) " ← PARK" else ""}")
                                dispatchGearChange(gear)
                            }
                            TX_PARKING_BRAKE -> {
                                data.enforceInterface(CAR_STATE_CALLBACK_DESCRIPTOR)
                                val pb = data.readInt()
                                log("CarState: parkingBrake → $pb")
                                dispatchParkingBrakeChange(pb)
                            }
                            TX_DOOR_SENSOR -> {
                                data.enforceInterface(CAR_STATE_CALLBACK_DESCRIPTOR)
                                val v = data.readInt()
                                log("CarState: door (tx=6) v=$v${if (v == DOOR_OPEN) " ← OUVERTE" else ""}")
            ...
            val proxy = java.lang.reflect.Proxy.newProxyInstance(
                ...
                // Pour les autres méthodes : valeurs par défaut primitives
            ...
            Log.d(TAG, "tryRegisterCarStateListener err: ${e.message}")
        }
    }

    /** Trigger si les deux conditions sont remplies (porte ouverte + gear=PARK) et armé.
     *  Le callback est différé du délai configurable pour laisser le temps au conducteur de sortir. */
    private fun maybeTriggerParkClose(reason: String) {
        if (sLastDoorSensor != DOOR_OPEN || sLastGear != GEAR_PARK) return
        val now = System.currentTimeMillis()
        val sinceLastTrigger = now - sLastTriggerTs
        if (sHasBeenDriving && sinceLastTrigger > TRIGGER_COOLDOWN_MS) {
            val delay = triggerDelayMs()
            log("$reason → TRIGGER programmé dans ${delay/1000}s")
            sLastTriggerTs = now
            sHasBeenDriving = false
            armDrivingTimer()
            val cbs = parkingStateCallbacks.toList()
            Handler(Looper.getMainLooper()).postDelayed({
                log("→ Fermeture (après délai ${delay/1000}s)")
                cbs.forEach { it() }
            }, delay)
        } else if (!sHasBeenDriving) {
            log("$reason ignoré (pas armé — <20 km/h et <5min)")
        }
    }
    ...
    private fun dispatchGearChange(gear: Int) {
        ...
        sLastGear = gear
        // Si on vient de passer en PARK et que la porte est déjà ouverte → trigger
        if (gear == GEAR_PARK) maybeTriggerParkClose("gear→PARK")
        val cbs = gearChangeCallbacks.toList()
        Handler(Looper.getMainLooper()).post { cbs.forEach { it(gear) } }
    }

    private fun dispatchParkingBrakeChange(pb: Int) {
        if (pb == sLastParkingBrake) return
        sLastParkingBrake = pb
        // Plus de trigger sur le frein de parking (trop de cas) — juste info
        val cbs = parkingBrakeCallbacks.toList()
        Handler(Looper.getMainLooper()).post { cbs.forEach { it(pb) } }
    }

    private fun dispatchDoorSensorChange(v: Int) {
        if (v == sLastDoorSensor) return
        sLastDoorSensor = v
        // Si la porte vient d'ouvrir et qu'on est en PARK → trigger
        if (v == DOOR_OPEN) maybeTriggerParkClose("door→OPEN")
    }
    ...
    /** Ferme toutes les vitres selon le mode configuré par vitre dans Settings.
     *  - AUTO : envoie cmd=3 une fois
     *  - PULSED : envoie cmd=1 toutes les pulseMs pendant durationMs
     *  - OFF : ne fait rien */
    fun closeAllWindowsPulsed(durationMs: Long = 3000L, pulseMs: Long = 120L) {
        ...
        log("Close: AUTO=$autoAreas  PULSED=$pulsedAreas  OFF=${Settings.ALL_AREAS - autoAreas - pulsedAreas}")

        // 1) Envoyer cmd=3 (AUTO_UP) une seule fois sur les vitres en mode AUTO
        ...
        // 2) Boucle de pulses cmd=1 sur les vitres en mode PULSED
    ...
    fun log(msg: String) {
        Log.i(TAG, msg)
        logLines.add(msg)
        while (logLines.size > 100) logLines.removeAt(0)
        onLogUpdated?.invoke()
    }
